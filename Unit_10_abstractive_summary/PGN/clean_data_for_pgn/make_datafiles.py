...
def chunk_file(set_name):
	...
	in_file = os.path.join(finished_files_dir,set_name + '.bin')
...
def tokenize_stories(stories_dir, tokenized_stories_dir):
	...
	# make IO list file
	mapfile = os.path.join(inputDir,"mapping.txt")
	print("Making list of files to tokenize...")
	with open(mapfile, "w") as f:
		for s in stories:
			f.write("%s \t %s\n" % (os.path.join(stories_dir, s), os.path.join(tokenized_stories_dir, s)))
	command = ['java', 'edu.stanford.nlp.process.PTBTokenizer', '-ioFileList', '-preserveLines', mapfile]
...
def write_to_bin(url_file, out_file, makevocab=False):
	...
	url_list = read_text_file(url_file,'rb')
...
if __name__ == '__main__':
	...
	try:
		opts, args = getopt.getopt(sys.argv[1:],"h:m:s:w:")
		...
		print(('python make_datafiles.py -m <mode: train.bin, test.bin, or val.bin> -s <folder containing story files> -w <working directory>'))
		
		sys.exit(2)
	
	#Initializing defaults:
	num_expected_stories = 0
	story_type = 'train.bin'
	inputDir = None
	subDir = "story_files"
	...
	all_urls = os.path.join(inputDir,"all_urls.txt") #This should be in the directory passed to -w
	#These directories should not exist yet or be empty:
	tokenized_stories_dir = os.path.join(inputDir,"tokenized_stories")
	finished_files_dir = os.path.join(inputDir,"finished_files")
	chunks_dir = os.path.join(finished_files_dir, "chunked")
	stories_dir = os.path.join(inputDir,subDir)
