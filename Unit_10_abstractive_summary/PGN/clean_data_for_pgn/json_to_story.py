...
def processJSON(jsonFile,outputDir,subDir):
    ...
    #at this point each line is a json dictionary for each article
    ...
    for line in lines:
        ...
        FILE.close()
        URL_FILE.write('%s\n' % url) #writes a line with a single URL per line in the same order as the files are numbered, for later disambiguation by make_datafiles.py
...
if __name__ == '__main__':
	...
	try:
	   opts, args = getopt.getopt(sys.argv[1:],"f:h:o:s:")
		...
		print(('python json_to_story.py -f <file name> -o <output directory> -s <folder name for story files>'))
